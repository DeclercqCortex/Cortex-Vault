...
// ===========================================================================
// Cluster 27 v1.0 — Interactive Plotter: sidecar JSON + arbitrary-file write.
// ===========================================================================
//
// Plots are stored as TipTap atom nodes in the markdown body
// (`<div data-cortex-plot>` with data-* attrs). For datasets larger than
// the inline threshold (50 rows × 4 cols) the actual data + config live
// in a sidecar JSON file at:
//
//   <parent-of-note>/<note-stem>-plots/<plot-id>.json
//
// This mirrors the Cluster 19 attachments directory and the Cluster 20
// shapes sidecar pattern: per-note subdirectory, JSON sidecar, idempotent
// writes, orphan GC.
//
// `write_text_file` is a small general-purpose helper used by the
// plotter's export buttons (CSV / SVG); keeping it cluster-27-tagged
// since this is its first consumer. Future clusters can reuse it.

#[derive(serde::Serialize, serde::Deserialize, Clone, Debug)]
pub struct PlotSidecarPayload {
    #[serde(default = "default_plot_schema_version")]
...
fn default_plot_schema_version() -> u32 {
    1
}

/// Compute the sidecar directory for a given note. Mirrors
/// `note_attachments_dir_for` but uses the `-plots` suffix.
fn note_plots_dir_for(note_path: &Path) -> Option<PathBuf> {
    let parent = note_path.parent()?;
    let stem = note_path.file_stem()?.to_string_lossy().to_string();
    if stem.is_empty() {
        return None;
    }
    Some(parent.join(format!("{}-plots", stem)))
}

/// Sanitize a plot id so it can safely be used as a filename. Plot IDs
/// generated by `generatePlotId()` in TypeScript are already in the
/// safe alphabet (alphanumeric + hyphen + underscore), but we belt-and-
/// suspenders this on the Rust side since the same id rides into a
/// filesystem path.
fn sanitize_plot_id(input: &str) -> String {
    input
        .chars()
...
fn plot_sidecar_path(vault_path: &str, note_path: &str, plot_id: &str) -> Result<PathBuf, String> {
    let vault = PathBuf::from(vault_path);
    ...
    let id = sanitize_plot_id(plot_id);
    if id.is_empty() {
        return Err("plot id is empty after sanitization".into());
    }
    let dir = note_plots_dir_for(&note)
        .ok_or_else(|| format!("couldn't compute plots dir for note `{}`", note_path))?;
    Ok(dir.join(format!("{}.json", id)))
}

/// Read a plot sidecar JSON file. Returns Ok(None) if the sidecar
/// doesn't exist yet (caller falls back to inline data or empty).
#[tauri::command]
fn read_plot_sidecar(
    vault_path: String,
    note_path: String,
    plot_id: String,
) -> Result<Option<PlotSidecarPayload>, String> {
    let path = plot_sidecar_path(&vault_path, &note_path, &plot_id)?;
    if !path.exists() {
        return Ok(None);
    }
    let raw = fs::read_to_string(&path)
        .map_err(|e| format!("read sidecar `{}` failed: {}", path.display(), e))?;
    let payload: PlotSidecarPayload = serde_json::from_str(&raw)
        .map_err(|e| format!("parse sidecar `{}` failed: {}", path.display(), e))?;
    Ok(Some(payload))
}

/// Write (or replace) a plot sidecar. Idempotent — only writes when
/// the serialized content differs from what's already on disk.
#[tauri::command]
fn update_plot_sidecar(
    vault_path: String,
    note_path: String,
    plot_id: String,
    payload: PlotSidecarPayload,
) -> Result<(), String> {
    let path = plot_sidecar_path(&vault_path, &note_path, &plot_id)?;
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .map_err(|e| format!("create plots dir `{}` failed: {}", parent.display(), e))?;
    }
    let serialized = serde_json::to_string_pretty(&payload)
        .map_err(|e| format!("serialize sidecar failed: {}", e))?;
    ...
    fs::write(&path, serialized)
        .map_err(|e| format!("write sidecar `{}` failed: {}", path.display(), e))?;
    Ok(())
}

/// Delete a plot sidecar. Non-fatal if the file doesn't exist (caller
/// might be cleaning up inline-mode transitions where there's nothing
/// to delete).
#[tauri::command]
fn delete_plot_sidecar(
    vault_path: String,
    note_path: String,
    plot_id: String,
) -> Result<(), String> {
    ...
    fs::remove_file(&path)
        .map_err(|e| format!("delete sidecar `{}` failed: {}", path.display(), e))?;
    // Clean up the plots dir if it's now empty (best-effort).
    if let Some(parent) = path.parent() {
...
/// Find plot sidecar files in the vault whose `plot-id` does not
/// appear in any markdown file's `data-plot-id` attribute. Mirrors
/// Cluster 19's `find_orphan_attachments`. Pass 2 wires this into
/// the Orphan-Attachments modal (or a sibling modal).
#[derive(serde::Serialize, Clone, Debug)]
pub struct OrphanPlot {
...
#[tauri::command]
fn find_orphan_plots(vault_path: String) -> Result<Vec<OrphanPlot>, String> {
    let vault = PathBuf::from(&vault_path);
    if !vault.is_dir() {
        return Err(format!("vault `{}` is not a directory", vault_path));
    }
    // Scan every <stem>-plots/ directory and every markdown file.
    let mut plot_files: Vec<(PathBuf, PathBuf, u64)> = Vec::new(); // (note, sidecar, bytes)
        ...
        if p.is_file() {
            ...
            if ext == "json" {
                if let Some(parent) = p.parent() {
                    ...
                    if let Some(stem) = parent_name.strip_suffix("-plots") {
                        let grandparent = parent.parent().unwrap_or(parent);
                        let note = grandparent.join(format!("{}.md", stem));
                        let size = p.metadata().map(|m| m.len()).unwrap_or(0);
                        plot_files.push((note, p.to_path_buf(), size));
                    }
                }
            } else if ext == "md" {
                if let Ok(body) = fs::read_to_string(p) {
                    // Cheap substring scan for `data-plot-id="…"`. Doesn't
                    // need to be HTML-aware; any occurrence (including
                    // inside an HTML comment) counts as a reference.
                    let needle = "data-plot-id=\"";
                    let mut search_from = 0;
                    while let Some(start) = body[search_from..].find(needle) {
                        let abs_start = search_from + start + needle.len();
                        if let Some(end_rel) = body[abs_start..].find('"') {
                            let id = &body[abs_start..abs_start + end_rel];
                            if !id.is_empty() {
    ...
    for (note, sidecar, size) in plot_files {
        let id = sidecar
            .file_stem()
            .and_then(|s| s.to_str())
            .unwrap_or("")
...
/// Write arbitrary text to a file at `path`. Used by the plotter's
/// CSV / SVG export. The caller picks the path via the dialog plugin
/// (so the user explicitly consents). We refuse writes outside the
/// vault only when the path looks suspicious (`..` traversal); the
/// dialog plugin already restricts to user-pickable locations.
#[tauri::command]
fn write_text_file(path: String, contents: String) -> Result<(), String> {
    ...
    fs::write(&pb, contents).map_err(|e| format!("write `{}` failed: {}", path, e))?;
    Ok(())
}

/// Pass 3 — write arbitrary bytes to a file at `path`. Companion to
/// `write_text_file`, used by the plotter's PNG export. Tauri marshals
/// Vec<u8> via a JSON array of small integers, which keeps the call
/// site simple at the cost of a few hundred extra bytes per kilobyte
/// of payload. Acceptable for plot exports (typically < 200 KB) and
/// avoids a separate base64 round-trip.
#[tauri::command]
fn write_binary_file(path: String, bytes: Vec<u8>) -> Result<(), String> {
    ...
    fs::write(&pb, &bytes).map_err(|e| format!("write `{}` failed: {}", path, e))?;
    Ok(())
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            // Cluster 27 v1.0 — Interactive Plotter
            read_plot_sidecar,
            update_plot_sidecar,
            delete_plot_sidecar,
            find_orphan_plots,
